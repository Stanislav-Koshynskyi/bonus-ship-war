...
public class Ship {
    ...
    public Ship(int size, int xStart, int yStart, ShipOrientation shipOrientation){
        ...
        if (xStart < 0 || xEnd >= Board.BOARD_SIZE)
            throw new IllegalArgumentException("Shep out of board by x, start - " +xStart + " , end - " + (xEnd >= Board.BOARD_SIZE));
        this.xStart = xStart;
        int yEnd = yStart + (shipOrientation == ShipOrientation.VERTICAL ? size - 1 : 0);
        if (yStart <0 || yEnd >= Board.BOARD_SIZE)
            throw new IllegalArgumentException("Shep out of board by y, start - " +yStart + " , end - " + (yEnd >= Board.BOARD_SIZE));
        this.yStart = yStart;
